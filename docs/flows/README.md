# Flows — Index

Пошаговые сценарии. Каждый flow ссылается на messages из [MESSAGE_CATALOG](../processes/MESSAGE_CATALOG.md).

| Flow | Doc |
|------|-----|
| Создать древо | [01-create-tree.md](./01-create-tree.md) |
| Зафиксировать версию | [02-commit-version.md](./02-commit-version.md) |
| Просмотр истории | [03-view-history.md](./03-view-history.md) |
| Прикрепить медиа | [04-attach-media.md](./04-attach-media.md) |
| Совладдение | [05-add-owner.md](./05-add-owner.md) |
| Восстановление | [06-recover-tree.md](./06-recover-tree.md) |
| Доступ по 12 словам | [07-seed-access.md](./07-seed-access.md) |
| Проводник сбора рода | [08-guided-tree.md](./08-guided-tree.md) |
| Pedigree UX | [09-pedigree-ux.md](./09-pedigree-ux.md) |
