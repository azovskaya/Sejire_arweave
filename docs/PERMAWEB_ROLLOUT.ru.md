...
## 8. Следующий конкретный вопрос к владельцу

Чтобы перейти от плана к коду/деплою, нужно решение по пунктам:

1. `sejire`: **lease** или **permabuy**?
2. Stripe: есть аккаунт (test keys)?
3. Cloudflare: аккаунт под Worker?
4. Treasury: отдельный hot JWK готов (не те же 12 слов, что личные)?

После ответов агент идёт по §5 без изменения модели безопасности §1–3.
