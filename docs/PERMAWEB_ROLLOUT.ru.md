...
| Stripe (оплата картой $3) | *ожидаем ответ* | |
| Cloudflare (служебная проверка оплаты) | *ожидаем ответ* | |
| Отдельный кошелёк проекта (казна) | *ожидаем ответ* | |

После ответов по Stripe / Cloudflare / казне агент идёт по §5 без изменения модели безопасности §1–3.
